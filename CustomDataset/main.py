...
def get_args_parser():
    ...
    parser.add_argument(
        "--data-set",
        default="CIFAR",
        choices=["CIFAR", "IMNET", "INAT", "INAT19", "image_folder", "PCAM"],
        type=str,
        help="Dataset to use (CIFAR, IMNET, INAT, INAT19, image_folder, PCAM)",
    )
    parser.add_argument(
        "--nb-classes",
        default=2,
    ...
    # test throught
    parser.add_argument(
        "--throughout", action="store_true", help="Perform throughout only"
    )
    return parser
...
def main(args):
    ...
    if args.eval:
        ...
        if "auc" in test_stats:
            print(
                f"AUC of the network on the {len(dataset_val)} test images: {test_stats['auc']:.4f}"
            )
        return
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        "MedViT training and evaluation script", parents=[get_args_parser()]
    )
