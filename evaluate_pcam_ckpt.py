...
def build_loader(data_root: str, split: str) -> data.DataLoader:
...
def load_model(model_name: str, ckpt_path: str) -> nn.Module:
...
def evaluate_one(model_name: str, ckpt_path: str, data_root: str) -> dict:
    ...
    for split in SPLITS:
        loader    = build_loader(data_root, split)
...
def parse_args():
    ...
    p.add_argument("--out_csv",   default="eval_pcam_results.csv",
...
def main():
    ...
    if args.ckpt:
        ...
        print(f"Sweeping {OUTPUT_ROOT}/*/pcam/{args.ckpt_name} ...")
        for model_name in MODELS:
            ckpt_path = os.path.join(OUTPUT_ROOT, model_name, "pcam", args.ckpt_name)
    ...
    for model_name, ckpt_path in jobs:
        try:
            ...
            rows.append({"model": model_name, "dataset": "pcam",
